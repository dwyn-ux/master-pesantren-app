    ...
    <div class="p-6 border-b border-gray-100">
        <form method="GET" class="flex flex-col md:flex-row gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('admin.ustadz.index') }}" class="px-6 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-medium transition-colors">
                    Reset
                </a>
            </div>
        </form>
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-indigo-50/30 transition-colors">
                    <td class="px-6 py-4 font-semibold text-gray-800">{{ $u->nama }}</td>
                    <td class="px-6 py-4 text-gray-600">{{ $u->no_hp ?? '-' }}</td>
                    <td class="px-6 py-4">
                        <span class="font-mono text-sm bg-gray-100 text-gray-600 px-3 py-1 rounded-md border border-gray-200">{{ $u->user->username }}</span>
                    </td>
                    <td class="px-6 py-4">
                        @if($u->user->is_active)
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            @if($u->user->is_active)
                                ...
                                <form method="POST" action="{{ route('admin.ustadz.toggle-status', $u) }}" class="inline-block" onsubmit="return confirm('Nonaktifkan ustadz {{ $u->nama }}?')">
                                    @csrf
                            ...
                            @else
                                <form method="POST" action="{{ route('admin.ustadz.toggle-status', $u) }}" class="inline-block" onsubmit="return confirm('Aktifkan kembali ustadz {{ $u->nama }}?')">
                                    @csrf
                                ...
                                <form method="POST" action="{{ route('admin.ustadz.destroy', $u) }}" class="inline-block" onsubmit="return confirm('Hapus permanen ustadz {{ $u->nama }}? Tindakan ini tidak bisa dibatalkan!')">
                                    @csrf @method('DELETE')
